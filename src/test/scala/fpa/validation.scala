...
class ValidationTests extends FlatSpec with Matchers {
  ...
  "Q1: Address (Line)" should "construct from valid values" in {
    "Street House #1".as[Address] shouldBe Valid(Address("Street House #1"))
    "  Street House #1  ".as[Address] shouldBe Valid(Address("Street House #1"))
  }
  "Q1: Address (Line)" should "not construct from invalid values" in {
    "\n\t\r ".as[Address] shouldBe Invalid(NEL.of("Empty address line"))
  }

  "Q2: PostalCode" should "construct from valid values" in {
  ...
  "Q2: PostalCode" should "not construct from invalid values" in {
    "  ".as[PostalCode] shouldBe Invalid(NEL.of("Empty postal code"))
  }


  "Q3: Name" should "construct from valid values" in {
    "Last, First".as[Name] shouldBe Valid(Name("First", "Last"))
    "  Last  ,  First  ".as[Name] shouldBe Valid(Name("First", "Last"))
  }
  "Q3: Name" should "not construct from invalid values" in {
  ...
  "Q4: Phone" should "construct from valid values" in {
  ...
  "Q4: Phone" should "not construct from invalid values" in {
  ...
  "Q5: Debit" should "construct from valid values" in {
  ...
  "Q5: Debit" should "not construct from invalid values" in {
    "".as[Debit] shouldBe Invalid(NEL.of("Invalid debit value: ''"))
    "1,00".as[Debit] shouldBe Invalid(NEL.of("Invalid debit value: '1,00'"))
  }


  "Q6: BirthDay" should "construct from valid values" in {
    val expected = Valid(BirthDay(LocalDate.of(1987, 1, 1)))
    "01/01/1987".as[BirthDay] shouldBe expected
  }
  "Q6: BirthDay" should "construct not from valid values" in {
    val expected = Invalid(NEL.of("Invalid birthday: '01-01-1987'"))
    "01-01-1987".as[BirthDay] shouldBe expected
  }


  "Q7: DebitRecord" should "construct from valid values" in {
    Source.fromFile("src/test/resources/data.csv").getLines().foreach { record =>
      record.as[DebitRecord] shouldBe a[Valid[_]]
    }
  }
  "Q7: DebitRecord" should "not construct from invalid values" in {
    import Fixture._
    faultyLine.as[DebitRecord] shouldBe Invalid(
      NEL.of(s"Invalid line pattern `${RecordPattern.regex}`: '$faultyLine'")
    )
    emptyLine.as[DebitRecord] shouldBe Invalid(
      NEL.of(
        "Invalid name: ','",
        "Empty address line",
        "Empty postal code",
